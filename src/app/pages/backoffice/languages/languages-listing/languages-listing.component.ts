...
@Component({
  ...
  templateUrl: './languages-listing.component.html',
  styleUrls: ['./languages-listing.component.scss'],
})
export class LanguagesListingComponent implements OnInit {
  ...
  deleteResource(resourceId: any) {
    this.serverBridgeService.deleteResource('/languages/', resourceId).subscribe(response => {
      this.getLanguages();
      this.translate.get('swal.delete-resource').subscribe(response => {
        if (response) {
          swal.fire(
